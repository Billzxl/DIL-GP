...
def get_dataset(dataset_name):
    ...
    elif dataset_name == "paris_housing":
        with open("uci_datasets/Regression with a Tabular Paris Housing Price Dataset/train.csv", "r") as f:
            data = f.readlines()
        
        data = [item.split(',') for item in data]
        labels = [item.strip() for item in data[0]]

        label_idx = labels.index("price")
        domain_indices = labels.index("cityCode")

        data = np.array(
            [[float(item.strip()) for item in items] for items in data[1:]]
        )

        cities = np.unique(data[:, domain_indices].astype(int))
        len_cities = np.zeros(len(cities))
        # print('cities', cities, len(cities))
        for i, city in enumerate(cities):
            len_city = len(np.where(data[:, domain_indices].astype(int) == city)[0])
            len_cities[i] = len_city

        ind = np.argpartition(len_cities, -100)[-100:]
        # print('len_cities[ind]', len_cities[ind], len_cities[ind].sum())

        train_indices = np.where(
            np.isin(data[:, domain_indices].astype(int), [cities[ind[i]] for i in range(50)])
        )[0]

        valid_indices = np.where(
            np.isin(data[:, domain_indices].astype(int), [cities[ind[i]] for i in range(50, 100)])
        )[0]

        data_np = np.concatenate(
            [data[:, 1:domain_indices], data[:, domain_indices+1:-1]], axis=1
        )
        target_np = data[:, -1]
