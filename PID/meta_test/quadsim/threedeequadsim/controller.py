...
class Controller():
    ...
    _name = None
    name_long = None
    def __init__(self, quadparamfile=DEFAULT_QUAD_PARAMETER_FILE, px4paramfile=DEFAULT_PX4_PARAM_FILE, update_pid=None):
        ...
        # 没有定义 update_pid 则默认值
        if update_pid is None:
            # print("PID value not set，pid = default.")
        ...
        else: # 定义了 update_pid 则为指定值
            # print("PID value set, pid = update_pid.")
            self.px4_params['angrate_gain_P'] = np.diag((update_pid['MC_ROLLRATE_P'],
                                                    update_pid['MC_PITCHRATE_P'],
                                                    update_pid['MC_YAWRATE_P']))
            self.px4_params['angrate_gain_I'] = np.diag((update_pid['MC_ROLLRATE_I'],
                                                    update_pid['MC_PITCHRATE_I'],
                                                    update_pid['MC_YAWRATE_I']))
            self.px4_params['angrate_gain_D'] = np.diag((update_pid['MC_ROLLRATE_D'],
                                                    update_pid['MC_PITCHRATE_D'],
                                                    update_pid['MC_YAWRATE_D']))
    ...
    def angrate(self, w, w_sp, dt, logentry):
        ...
        # print(self.px4_params['angrate_gain_P'])
