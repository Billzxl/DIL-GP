...
class OODGPKernel(nn.Module):
    ...
    def forward(self, x):
        ...
        L = self.L
        alpha = self.alpha
        k = self.kernel(self.X, x).to(L.device)
    ...
    def fit(self, X, y):
        ...
        for i in range(ei_step):
            ...
            K_detach = self.kernel.forward_w_scale(X, X, self.scale, detach=True).to(X.device) + (self.noise_scale.detach() * self.scale) * torch.eye(len(X), device=X.device)
            try:
        ...
        self.scale = torch.tensor(1., device=X.device).requires_grad_()
        K = self.kernel.forward_w_scale(X, X, self.scale).to(X.device) + (self.noise_scale * self.scale) * torch.eye(len(X), device=X.device)
        try:
